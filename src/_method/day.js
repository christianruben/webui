...
async function insert(data, callback){
    ...
    try{
        let response = await fetch(`${config.endpoint}/day`, config.postdataconfig(data))
        let json = await response.json()
        if(response.status == 200){
...
async function del(id, callback){
    ...
    try{
        let response = await fetch(`${config.endpoint}/day`, config.deletedataconfig({id: id}))
        let json = await response.json()
        if(response.status == 200){
            result.json = json.data
        }else{
...
async function update(data, callback){
    ...
    try{
        let response = await fetch(`${config.endpoint}/day`, config.putdataconfig(data))
        let json = await response.json()
        if(response.status == 200){
            result.json = json.data
        }else{
