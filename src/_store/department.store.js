import {department} from '../_method'
...
export const departments = {
    ...
    actions: {
        storeReq({commit}, {index, rows, search, sortby, sort}){
            ...
            department.list({index: index, rows: rows, search: search, sortby: sortby, sort: sort}, (result)=>{
        ...
        searchLight({commit}, search){
            department.lightsrc(search, (result)=>{
                const {json} = result
                setTimeout(()=>{
        ...
        uploadDepartment({commit}, {data}) {
            commit('removeError')
            commit('setLoading', true)
            commit('setUpload', true)
            department.insert(data, (result)=>{
                const {err, json} = result;
                if(err){
                    commit('setError', err)
                    commit('setUpload', true)
