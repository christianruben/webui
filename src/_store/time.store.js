...
export const times = {
    ...
    actions: {
        storeReq({commit}, {index, rows, search, sortby, sort}){
            ...
            time.list({index: index, rows: rows, search: search, sortby: sortby, sort: sort}, (result)=>{
        ...
        uploadTime({commit}, {data}) {
            commit('removeError')
            commit('setLoading', true)
            commit('setUpload', true)
            time.insert(data, (result)=>{
                const {err, json} = result;
                if(err){
                    commit('setError', err)
                    commit('setUpload', true)
