...
<script>
  export default {
    components: {
      FormSchedule: () => import('@/components/FormSchedule'),
    ...
    data () {
      return {
        ...
        theaders: [
          ...
          {text: 'Jam pelajaran', value: 'study_time', sortable: false},
    ...
    methods: {
      ...
      OkButton(){
        const {dispatch} = this.$store;
        dispatch('schedules/deleteSchedule', {id: this.table[this.idselected].schedule_id})
        dispatch('schedules/deleteItems', {id: this.idselected})
        this.alert = false
        this.idselected = 0
      },
      NoButton(){
